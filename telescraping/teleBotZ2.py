...
def find_the_song(message):
    url = f'https://z2.fm/mp3/search?keywords={message.text}'
    html = get_html(url)
    soup = get_soup(html)
    try:
        singerMain = soup.find('div', class_='whb_gr_l')
    except:
        chat_id = message.chat.id
...
def get_data_songs(singerMain):
    res_search = []
    singer = singerMain.find_all('div', class_='song-wrap song-wrap-xl')
    for item in singer:
        song_artist = item.find('div', class_='song-artist').find('a', class_='mb-tooltip').find('span').text
        song_name = item.find('div', class_='song-name').find('a', class_='mb-tooltip').find('span').text
        song_url = 'https://z2.fm'+item.find('span', class_='song-play btn4 play').get('data-url')
        data = {
                'artist': song_artist,
                'name': song_name,
                'url': song_url
...
def main():
    ...
    #     good_img = item.find('div', class_='product-item__content').find('picture').find('img').get('data-src')
    #     good_img = good_img.replace('//', '')
    #     print(good_img)
